...
const configureRouter = (data) => {
    ...
    router.post('/', async (req, res) => {
        ...
        try {
            ...
            data.emit('productData', savedProduct);
        } catch (err) {
            res.status(500).json({ msg: err.message });
        }
    });
    ...
    router.put('/:pid', async (req, res) => {
        try {
            ...
            if (updatedProduct) {
                ...
                data.emit('productData', updatedProduct);
            } else {
                res.status(404).json({ msg: 'Product not found' });
            }
        } catch (err) {
    ...
    router.delete('/:pid', async (req, res) => {
        try {
            ...
            if (removedProduct) {
                ...
                data.emit('productRemoved', { id: req.params.pid });
                data.emit('cartUpdated');
